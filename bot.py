...
def StartBot():
    ...
    async def MenuChoise(message: types.Message):
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton(text="Карта корпусов", callback_data="menu_map"))
    ...
    async def menu_messages(call: types.CallbackQuery):
        if call.data == "menu_map":
            photo = InputFile('map.jpg', 'rb')
            await call.message.answer_photo(photo, caption="Карта корпусов")
        elif call.data == "menu_rasp":
    ...
    async def send_welcome(message: types.Message):
        await message.answer("Привет! Это чат-бот, который поможет тебе освоиться в ОмГТУ.")
